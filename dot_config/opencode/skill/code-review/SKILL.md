...
For each finding, show which agents have found it. It's important to know if a specific finding was found by multiple agents.
...
- @backend-expert - Should be consulted about the architecture of a backend project (queues, databases, etc.). Only consult this when working on a **backend service**, not when working on CLIs or scripts
